...
by partitioning the event $\Tsys > t$ according to the number of components of type $k$ functioning at time $t$,
denoted by $C^k_t \in \{0, 1, \ldots, l_k, \ldots, m_k\}$, and applying the law of total probability:
\begin{align*}
P(\Tsys > t)
...
we now need to minimise and maximise Equation~*** over $\PtZi{1}, \ldots, \PtZi{K}$ for each $t$,
...
With (strongly) differing $\nktz$ values, it can happen that, e.g., $\ynu_{k,t_j} < \ynu_{k,t_{j+1}}$!\\
E.g., when $\yzu_{k,t_j} = \yzu_{k,t_{j+1}}$ and $\yzl_{k,t_j} = \yzl_{k,t_{j+1}}$,
$s^k_t/n_k \in [\yzl_{k,t_j}, \yzu_{k,t_j}]$ (so no prior-data conflict) and no failures in $[t_j, t_{j+1}]$,\\
then with $\nzu_{k,t_j} < \nzu_{k,t_{j+1}}$ we have $\ynu_{k,t_j} < \ynu_{k,t_{j+1}}$,\\
 and with $\nzu_{k,t_j} > \nzu_{k,t_{j+1}}$ we have $\ynl_{k,t_j} > \ynl_{k,t_{j+1}}$.\\
(weighted average between interval and precise value: the lower $n$, the 'faster' the interval will shrink!)\\
So we need to require same $\nktz$ bounds for all $t$!
