...
straightforward computation of system relaibility $\Rsys(t) = P(\Tsys > t)$
...
is binomial distributed, $C^k_t = \sum_{i=1}^{m_k} \indic(T^k_i > t) \sim \bin(p^k_t, m_k)$.
...
The $p^k_t$'s could be such that they reflect, e.g., a bathtub curve for the corresponding hazard rate.
The only strict constraint is that $p^k_{t_j} \ge p^k_{t_{j+1}}$ must hold (assuming no repair).

However, directly choosing $p^k_t$'s is hard,
and fixing values for all $p^k_t$ would furthermore neglect any inherent uncertainty in the particular choice.  
To account for this uncertainty, one can express knowledge on $p^k_t$'s though a prior distribution.
A convenient and natural choice is $p^k_t \sim \be(\alpha^k_t, \beta^k_t)$.

in terms of $\nz$, $\yz$

posterior, posterior predictive for $C^k_t$

***Illustration prior-data conflict?
