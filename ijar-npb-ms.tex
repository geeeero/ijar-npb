...
in a no-conflict situation, when $\nktzu = n_k$ $[\yktnl, \yktnu]$ has half the length of $[\yktzl, \yktzu]$.
...
as is clear from Figure~***, the wider the $\nktz$ interval,
...
it is not advisable to choose $\yktzl = 0$ and $\yktzu = 1$.
For any $t \not\in \big(\min(\vec{t}^k), \max(\vec{t}^k)\big)$,
this can lead to improper posterior predictive distributions if $l_k=0$ or $l_k=m_k$.
...
We rather propose to elicit bounds only for a subset of ${\cal T}$,
and to fill up the $t$ grid with the least committal bounds, i.e.,
taking $\yktzu$ equal to last (in the time sequence) elicited $\yktzu$,
an likewise $\yktzl$ equal to next (in time sequence) elicited $\yktzl$.
...
and then to further refine the obtained bounds if necessary.
%`middle' or `typical' $t$,
...
The method is implemented in the \textbf{R} package \texttt{ReliabilityTheory} \citep{2015:aslett-RT}
with the function \texttt{nonParBayesSystemInferencePriorSets()};
the package also contains the function \texttt{computeSystemSurvivalSignature()}
to calculate the survival signature for a given system layout (reliability block diagramm).
...
\node[type1] (T1)   at ( 0  , 0) {T1};
\node[type2] (T2)   at ( 2.8, 0) {T2};
\node[type3] (T3-2) at ( 1.4, 1) {T3};
\node[type3] (T3-3) at ( 1.4,-1) {T3};
\node[type3] (T3-5) at ( 4.2, 1) {T3};
\node[type3] (T3-6) at ( 4.2,-1) {T3};
\coordinate (start) at (-1,0);
\coordinate (end)   at (5.6,0);
\coordinate (bista) at (0.7,0);
\coordinate (biend) at (4.9,0);
\path (start)     edge[thick]   (T1.west)
      (T1.east)   edge[thick]   (bista)
      (bista)     edge[vh path] (T3-2.west)
                  edge[vh path] (T3-3.west)
      (T3-2.east) edge[hv path] (T2.north)
      (T3-3.east) edge[hv path] (T2.south)
      (T2.north)  edge[vh path] (T3-5.west)
      (T2.south)  edge[vh path] (T3-6.west)
      (T3-5.east) edge[hv path] (biend)
      (T3-6.east) edge[hv path] (biend)
      (biend)     edge[thick]   (end);
%\end{scope}
\end{tikzpicture}
...
For components type T1 and T2, we consider a near-noninformative set of prior reliability functions.
...
test data for components of type T1 and T2 are invariably
$\vec{t}^1 = (2.0, 2.2, 2.4, 2.6)$ and $\vec{t}^2 = (3.0, 3.2, 3.4, 3.6)$, respectively.
...
due to the prior lower bound for T1;
for the system to function, the component of type T1 must function.
