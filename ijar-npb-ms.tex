...
to bounds on the posterior predictive distribution for the reliability of
...
There are new results on first-order stochastic dominance for the Beta-Binomial distribution
which make the technique computationally tractable.  These methodological contributions can be immediately used in
applications by reliability practitioners using the freely available
software tools which are provided.
\end{abstract}
...
ones beliefs with the exactness a prior requires.  Finally,
...
a nonparametric method which utilises imprecise probability 
to encapsulate more vague or imperfect prior beliefs using
...
framework and leads to an easy method of detecting prior-data
conflict which should be of concern at the system level.  

Furthermore, the method is based on the survival signature, a 
recent development which naturally accommodates 
heterogeneous component types laid out in any arbitrary
manner which maintains coherency of the system.  As will be 
seen in the sequel, the method remains computationally tractable.
...
formulation of the system reliability bounds and which leads to
