...
the reparametrisation of that approach which enables the natural
...
\cite{1996:walley::IDM} for Dirichlet priors,
...
in 91.9\% of cases for $\lRsys(\cdot)$ and in 95.7\% of cases for $\uRsys(\cdot)$.
Further detail is in Appendix \ref{ap:needtoopt-brake}.
...
Thus, Figures \ref{fig:need-to-opt-1}, \ref{fig:need-to-opt-2} and \ref{fig:need-to-opt-3} show how often optimisation would be required
under long term repeated use of this technique.
...
Clearly the true functioning probability, $p$, is unknown.  Figure \ref{fig:need-to-opt-45} therefore shows the worst case scenario for any choice of $y$ --- that is, where the maximum of each curve from the previous subsection is taken.
...
Note that all these analyses are for evaluation of a single fixed time point at which $\lRsys(\cdot)$ or $\uRsys(\cdot)$ is computed.  Of relevance to the overall compute time is the behaviour over time, and it seems unlikely that the worst case scenario will occur for long runs of the times under analysis.

\subsection{Theoretical analysis for brake system example}
\label{ap:needtoopt-brake}

Such temporal aspects are now considered by analysing the brake system example from Section~\ref{sec:brakeex}.
%By way of example, a temporal analysis of the constrained worst case analysis is now presented for the example in Section~\ref{sec:brakeex}.
%
...
As noted in Section~\ref{sec:brakeex}, the grid contains 301 time points and the system has 4 types of components,
meaning stochastic dominance must be verified via theory or optimisation $1,204$ times for each of $\lRsys(\cdot)$ and $\uRsys(\cdot)$.

Figure~\ref{fig:need-to-opt-8} shows the worst case probabilities (subject to $|y-p|>0.1$)
as they change over time in the brake system example. %from the main body of the paper.
Displayed are the worst case probabilities that optimisation will be required for the choice of prior and the values of $m$ and $N$
as in Section~\ref{sec:brakeex}, but without using the test data (which define $s$).
Note in particular that when $m=1$ (as is the case for component types M and H),
Lemma 3 always applies for $y \in (0,1)$ and so optimisation is never required.
This is true for all values of $N$ and $y$.
...
For the test data as used in Section~\ref{sec:brakeex},
theory could not determine stochastic dominance in 98 (or 8.1\%) of cases for $\lRsys(\cdot)$,
while it was in 52 (or 4.3\%) of cases for $\uRsys(\cdot)$.
